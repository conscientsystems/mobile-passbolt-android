...
class ServerReachabilityTracker {
    ...
    fun unreachableSince(sinceNanos: Long): Boolean {
        val unreachable = lastUnreachableNanos ?: return false
        if (unreachable - sinceNanos < 0) {
            return false
        }
        val reachable = lastReachableNanos ?: return true
        return reachable - unreachable < 0
    }
}
