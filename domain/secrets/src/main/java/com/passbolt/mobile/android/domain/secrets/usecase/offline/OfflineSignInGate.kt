...
class OfflineSignInGate(
    ...
    suspend fun evaluate(userId: String): Result {
        ...
        if (!flags.offlineMode.isEnabled) {
            return Result.Disabled
        }
        val lastSync = flags.offlineLastSyncEpochMillis?.let { Instant.ofEpochMilli(it) } ?: return Result.NoCache
        val now = Instant.now()
        // a clock set back in time is treated as expired, never as "fresh again"
        if (now.isBefore(lastSync) || Duration.between(lastSync, now) > DATA_RETENTION) {
        ...
        if (offlineCacheRepository.countCachedSecrets(userId) == 0) {
            return Result.NoCache
        }
        return Result.Allowed(lastSyncEpochMillis = lastSync.toEpochMilli())
    }
