 ...
 * @since v1.0
 */
class UpdateHomeDisplayViewPreferencesUseCase(
    private val accountPreferencesRepository: AccountPreferencesRepository,
    private val getSelectedAccountUseCase: GetSelectedAccountUseCase,
) : UseCase<HomeDisplayViewPreferencesUpdate, Unit> {
    override fun execute(input: HomeDisplayViewPreferencesUpdate) {
        val userId = requireNotNull(getSelectedAccountUseCase.execute(Unit).selectedAccount)
        accountPreferencesRepository.updateHomeDisplayViewPreferences(input, userId)
    }
}
