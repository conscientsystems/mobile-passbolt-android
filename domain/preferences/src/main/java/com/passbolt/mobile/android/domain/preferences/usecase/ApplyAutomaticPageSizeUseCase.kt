...
class ApplyAutomaticPageSizeUseCase(
    ...
    override fun execute(input: Unit) {
        ...
        if (globalPreferences.isApiFetchPageSizeManuallySet) {
            Timber.d("Api fetch page size is set manually; keeping the user preference")
            return
        }
        val automaticPageSize = getAutomaticPageSizeUseCase.execute(Unit).defaultPageSize
        if (automaticPageSize != globalPreferences.apiFetchPageSize) {
            Timber.d("Applying automatic api fetch page size of %d", automaticPageSize)
            globalPreferencesRepository.updateGlobalPreferences(
                GlobalPreferencesUpdate(apiFetchPageSize = automaticPageSize),
            )
        }
    }
}
