 ...
 * @since v1.0
 */
class GetHomeDisplayViewPreferencesUseCase(
    private val accountPreferencesRepository: AccountPreferencesRepository,
    private val getSelectedAccountUseCase: GetSelectedAccountUseCase,
) : UseCase<Unit, HomeDisplayViewPreferencesUiModel> {
    override fun execute(input: Unit): HomeDisplayViewPreferencesUiModel {
        val userId = requireNotNull(getSelectedAccountUseCase.execute(Unit).selectedAccount)
        return accountPreferencesRepository.getHomeDisplayViewPreferences(userId)
    }
}
