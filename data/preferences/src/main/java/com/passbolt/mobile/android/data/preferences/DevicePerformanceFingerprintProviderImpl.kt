...
) : DevicePerformanceFingerprintProvider {
    override fun provideFingerprint(): DevicePerformanceFingerprint {
        val memoryInfo = ActivityManager.MemoryInfo().also(activityManager::getMemoryInfo)
        return DevicePerformanceFingerprint(
            isLowRamDevice = activityManager.isLowRamDevice,
            totalMemMb = memoryInfo.totalMem / BYTES_IN_MEBIBYTE,
            memoryClassMb = activityManager.memoryClass,
        )
    }

    private companion object {
        private const val BYTES_IN_MEBIBYTE = 1024L * 1024L
    }
}
