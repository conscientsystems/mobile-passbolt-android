...
internal fun Group.toEntity(): UsersGroup =
    UsersGroup(
        groupId = id,
        name = name,
    )
...
internal fun UsersGroupWithChildItemsCount.toDomain(): GroupWithItemsCount =
    GroupWithItemsCount(
        id = groupId,
        name = name,
        itemsCount = childItemsCount,
    )

internal fun GroupWithUsersEntity.toDomain(): GroupWithUsers =
    GroupWithUsers(
        group = group.toDomain(),
        users = users.map { it.toUserProfile() },
    )
